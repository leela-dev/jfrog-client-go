...
async function RunFrogbot(inputs) {
    ...
    tasks.info("Running frogbot scan with:"+binary+" "+inputs.botAction)
    try {
        const path = tasks.getVariable("EXECUTION_PATH")
        await VerifyFrogbotInstallation(path, binary)
...
async function VerifyFrogbotInstallation(path, binary) {
    let {stdOut, stdErr} = (await tasks.execute(`cd ${path} && ${binary} --help`))
    tasks.info(stdErr)
    tasks.info(stdOut)
}
